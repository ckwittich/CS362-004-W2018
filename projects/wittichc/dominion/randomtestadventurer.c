...
int main(int argc, char const *argv[])
{
	...
	while(i < 100000)
	{
		...
		//check number of treasures in hand before adventurerPlay is called
		numTHB = 0;
		j = 0;
		...
		adventurerPlay(game, 0);

		//check number of treasures in hand after adventurerPlay is called
		numTHA = 0;
		j = 0;
		while(j < game->handCount[0])
		{
		...
		else if(numT < 2 && numTHA == numTHB + numT)
		{
			fprintf(fp, "Test %d PASSED.\n", i + 1);
			pass++;
		}
